...
@bp_events.post("/")
@bp_events.post("")  # ✅ accepte maintenant /events (sans slash) ET /events/
@login_required
def create_event():
    ...
    ev = Event(
        ...
        created_by_id=current_user.id,  # ton modèle utilise bien *_id (colonne FK)
    )
    db.session.add(ev)
...
def event_verify(event_id: int):
    """Création d’un enregistrement de vérification (ITEM)."""
    if not _is_manager():
        abort(403)
    ev = _event_or_404(event_id)
