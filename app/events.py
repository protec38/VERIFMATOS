...
from .models import (
    db, Item, Event, EventItem, EventChild, Verification, Activity, Presence,
...
def create_event():
    ...
    if request.method == 'POST':
        ...
        # associer parents + préparer enfants inclus & vérifs
        for pid in parent_ids:
            ...
            for child in p.children:
                ...
                # créer aussi la ligne de vérification
...
# ---------------------------------------------------------------------
# Vue chef/admin + Page de partage
# ---------------------------------------------------------------------
...
def view_event(event_id):
    ...
    # prépare (parent, [enfants inclus]) pour le template
    tree = []
    for p in parents:
        included_children = []
        for c in p.children:
...
def verify(token):
    ...
    for p in parents:
        ...
        for c in p.children:
            ec = EventChild.query.filter_by(event_id=ev.id, child_id=c.id).first()
            if ec and ec.included:
                # s'assurer que la ligne Verification existe
                v = Verification.query.filter_by(event_id=ev.id, item_id=c.id).first()
                if not v:
                    db.session.add(Verification(event_id=ev.id, item_id=c.id, verified=False))
                children.append(c)
...
# ---------------------------------------------------------------------
# Présence (badge "en cours" sur un parent)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# APIs status / verify / load (AJAX)
# ---------------------------------------------------------------------
def _status_payload(ev: Event) -> dict:
    # map verifs (item enfant -> état)
    verifs = Verification.query.filter_by(event_id=ev.id).all()
    vmap = {v.item_id: v for v in verifs}

    # statut par parent = tous ses enfants inclus sont ok
    parents_status = {}
    loaded = {}
    for ei in ev.event_items:  # chaque parent associé à l'évènement
...
def api_load(event_id):
    ...
    # enfants inclus pour CE parent uniquement
    ecs = EventChild.query.filter_by(event_id=ev.id, parent_id=parent_id, included=True).all()
    child_ids = [ec.child_id for ec in ecs]
    ...
    db.session.add(Activity(event_id=ev.id, actor=current_user.username, action=('load' if want_loaded else 'unload'), item_id=parent_id))
    db.session.commit()
...
def export_pdf(event_id):
    ...
    for parent in parents:
        ...
        pdf.drawString(40, y, f"Parent: {parent.name}")
        y -= 14
        ...
        for child in parent.children:
            ...
            pdf.drawString(60, y, f"- {child.name} (x{child.expected_qty})  [{status}]  {who} {when}")
            y -= 12
        y -= 6

    pdf.setFont("Helvetica", 10)
    pdf.drawString(40, 60, "Chef de poste: ____________________")
    pdf.drawString(300, 60, "Conducteur: ____________________")
