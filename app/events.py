...
@login_required
def list_events():
    if not is_admin_or_chef(): return redirect(url_for('index'))
...
@login_required
def create_event():
    if not is_admin_or_chef(): return redirect(url_for('events.list_events'))
    if request.method=='POST':
        title=request.form.get('title'); date=request.form.get('date'); location=request.form.get('location')
        parent_ids = [int(x) for x in request.form.getlist('parent_ids')]
        ev=Event(title=title, date=datetime.fromisoformat(date) if date else datetime.utcnow(), location=location, chef_id=current_user.id, state='in_progress')
        ...
        db.session.commit(); flash('Évènement créé','success'); return redirect(url_for('events.view_event', event_id=ev.id))
...
def edit_event(event_id):
    ...
    if not is_admin_or_chef(): return redirect(url_for('events.list_events'))
    if ev.state=='closed':
        flash('Évènement clôturé — modification interdite.','warning'); return redirect(url_for('events.view_event', event_id=ev.id))
    if request.method=='POST':
...
def edit_children(event_id):
    ...
    if request.method=='POST':
        ...
        db.session.commit(); flash('Enfants mis à jour','success'); return redirect(url_for('events.view_event', event_id=ev.id))
    # build tree
    ...
    for p in parents:
        ...
        for c in children:
            ...
            if not ec:
                ec=EventChild(event_id=ev.id, parent_id=p.id, child_id=c.id, included=True); db.session.add(ec); db.session.commit()
...
def change_state(event_id):
    ...
    if target not in ('draft','in_progress','closed'): return redirect(url_for('events.view_event', event_id=ev.id))
    ev.state=target; db.session.commit(); flash(f"État passé à {target}",'success')
...
def token_entry(token):
    ...
    if ev.state=='closed':
        flash('Évènement clôturé','warning'); return redirect(url_for('events.token_entry', token=token))
    if request.method=='POST':
        full_name=request.form.get('full_name','').strip()
        if not full_name: flash('Nom et prénom requis','warning'); return redirect(url_for('events.token_entry', token=token))
        session['volunteer_name']=full_name; session['event_token']=token
...
def verify(token):
    ...
    # build tree using EventChild.included
    parents=[ei.item for ei in ev.event_items]
    data=[]
    for p in parents:
        children=[c for c in p.children if (EventChild.query.filter_by(event_id=ev.id, child_id=c.id).first() and EventChild.query.filter_by(event_id=ev.id, child_id=c.id).first().included)]
...
def ping_presence(token):
    ev=Event.query.filter_by(token=token).first_or_404()
    name=session.get('volunteer_name'); parent_id=int((request.get_json() or {}).get('parent_id') or 0)
    if not name: return jsonify({'ok':False}), 401
    ...
    if not pr:
        ...
        db.session.add(pr)
    pr.ping_at=datetime.utcnow(); db.session.commit(); return jsonify({'ok':True})
...
def _status_payload(ev):
    ...
    parents_status={}
    for p in parents:
        child_ids=[c.id for c in p.children if (EventChild.query.filter_by(event_id=ev.id, child_id=c.id).first() and EventChild.query.filter_by(event_id=ev.id, child_id=c.id).first().included)]
    ...
    return {'verifications': {str(v.item_id): {'verified': v.verified, 'by': v.last_by, 'at': v.last_at.isoformat() if v.last_at else None} for v in verifs},
            'parents_complete': parents_status, 'loaded': loaded, 'busy': busy }

@events_bp.route('/api/<int:event_id>/status')
def api_status(event_id):
    ev=Event.query.get_or_404(event_id); return jsonify(_status_payload(ev))

@events_bp.route('/api/token/<token>/status')
def api_status_token(token):
    ev=Event.query.filter_by(token=token).first_or_404(); return jsonify(_status_payload(ev))

@events_bp.route('/api/<token>/verify', methods=['POST'])
def api_verify(token):
    ev=Event.query.filter_by(token=token).first_or_404()
    if ev.state=='closed': return jsonify({'ok':False,'error':'closed'}), 400
    name=session.get('volunteer_name')
    if not name: return jsonify({'ok':False,'error':'auth'}), 401
    data=request.get_json() or {}; item_id=int(data.get('item_id')); state=bool(data.get('verified'))
    v=Verification.query.filter_by(event_id=ev.id, item_id=item_id).first()
    if not v: v=Verification(event_id=ev.id, item_id=item_id, verified=state, last_by=name, last_at=datetime.utcnow()); db.session.add(v)
    else:
        v.verified=state; v.last_by=name; v.last_at=datetime.utcnow()
...
def api_load(event_id):
    ev=Event.query.get_or_404(event_id)
    if not is_admin_or_chef(): return jsonify({'ok':False,'error':'forbidden'}),403
    data=request.get_json() or {}; item_id=int(data.get('item_id')); loaded=bool(data.get('loaded'))
    # block loading until all children verified
...
def export_csv(event_id):
    ...
    cw.writerow([]); cw.writerow(['ItemID','Parent','Child','ExpectedQty','Verified','By','At','LoadedParent'])
    loaded_map={ei.item_id: ei.loaded for ei in ev.event_items}
    parents=[ei.item for ei in ev.event_items]
    for p in parents:
        for c in p.children:
            ec=EventChild.query.filter_by(event_id=ev.id, child_id=c.id).first()
            if not ec or not ec.included: continue
    ...
    mem=io.BytesIO(si.getvalue().encode('utf-8')); mem.seek(0); return send_file(mem, mimetype='text/csv', as_attachment=True, download_name=f"event_{ev.id}.csv")

@events_bp.route('/<int:event_id>/export.pdf')
@login_required
def export_pdf(event_id):
    ev=Event.query.get_or_404(event_id)
    mem=io.BytesIO(); p=canvas.Canvas(mem, pagesize=A4); w,h=A4; y=h-40
...
def export_log(event_id):
    ...
    mem=io.BytesIO(si.getvalue().encode('utf-8')); mem.seek(0); return send_file(mem, mimetype='text/csv', as_attachment=True, download_name=f"event_{ev.id}_log.csv")
